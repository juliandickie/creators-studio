...
**Three scripts exist** (all in `scripts/dev/`, no extra deps beyond `git`/`gh`/`jq`/`zip` and stdlib Python):
...
| `scripts/dev/check_live_config.py` | Fails if a live config file references something that does not exist. Runs in CI; run it locally any time you rename or move a directory. |
...
| `skills/create-image/presets/*.json` | 12 example brand guide presets. Copy to `~/.creators-studio/presets/` (v4.2.2+; auto-migrates from `~/.banana/presets/`). |
...
| `skills/create-image/scripts/setup_mcp.py` | MCP + Replicate key configuration. Stores keys in `~/.creators-studio/config.json` (v4.2.2+; auto-migrates from `~/.banana/config.json`). |
...
| `skills/create-transcript/SKILL.md` | **v4.3.0** Transcript Creative Director orchestrator for `/create-transcript`. Intent → format defaults, language auto-detect, speaker-naming prompt, chapter-naming split responsibility (script finds boundaries, Claude titles them), keyterm confirm, verify-before-done. |
| `skills/create-transcript/scripts/transcribe.py` | **v4.3.0 (keyterm sets v4.4.0)** Scribe v2 speech-to-text CLI. Reuses the ElevenLabs key, `_http_post_multipart` (repeated `keyterms` fields), ffprobe/ffmpeg mono-16kHz prep (fail-loud on no-audio), one STT call, JSON cache, render dispatch, plus `rename`/`cost`/`status` subcommands and batch. v4.4.0 adds per-video keyterm sets: `resolve_keyterm_sets()` expands named `transcription.keyterm_sets` via `--keyterm-set`, `resolve_keyterms()` unions ad-hoc + sets + always-on base + default (`--keyterms-replace` = terms named this run only). v4.5.0 adds descriptive titles: `render_markdown` gains an optional `title` (H1), the `retitle` subcommand + `--title` flag set the H1 and prefix output filenames with `<title> - ` (video name kept at the tail), `_title`/`_source_name` persisted in cache for idempotent re-runs. Stdlib only. Pure helpers (auth/keyterms/sets/probe/multipart) are unit-tested. |
| `skills/create-transcript/scripts/formats.py` | **v4.3.0** Pure renderers (no network/fs): markdown (speaker turns), SRT + VTT (cue segmentation, independent of speakers), chapters (pause boundaries + placeholder titles), plaintext. The cached JSON is the single source every format re-renders from. |
| `skills/create-transcript/references/scribe-models.md` | **v4.3.0** Scribe v2 roster, endpoint + fields, constraints, audio-prep rationale, response shape, subscription billing + keyterm surcharge/min-billable notes. |
| `skills/create-transcript/references/transcript-formats.md` | **v4.3.0** The five formats, timecode styles, turn-grouping vs cue-segmentation, chapter split responsibility, speaker naming. |
| `skills/create-transcript/references/keyterms.md` | **v4.3.0** Keyterm prompting: repeated-field API encoding, client-side sanitisation rules, three-tier precedence, cost note, recommended standing config list. |
...
| `tests/test_*.py` | **v4.2.0** Test suite (stdlib `unittest`, zero pip deps, 247 tests). Run with `python3 -m unittest discover tests`. HTTP mocked via `urllib.request.urlopen` patch; no network required. Fixtures at `tests/fixtures/*.json`. |
...
| `scripts/paths.py` | **v4.2.2** Single source of truth for the user state directory. Exposes `creators_studio_dir()` (returns `~/.creators-studio/`, auto-migrating from `~/.banana/` via `shutil.copytree(symlinks=True)` on first call) plus 8 convenience accessors (`config_path`, `costs_path`, `presets_dir`, `history_dir`, `assets_dir`, `ab_preferences_path`, `ab_history_dir`, `analytics_path`). Migration is COPY (not move) - old dir preserved for safety. Plus `migration_status()` for `validate_setup.py --check-migration`. Stdlib only. Tests at `tests/test_paths.py` cover all four state-graph corners. |
...
| `scripts/dev/check_live_config.py` | **2026-07-27** Guards against the v4.0.0-rename bug class: live config that references a path which no longer exists and fails silently. Checks CODEOWNERS (every rule must match at least one tracked file), `.github/dependabot.yml` (every declared ecosystem must have a manifest to track) and `marketplace.json` (local `source` must resolve to a real plugin). Emits `::error file=,line=` annotations in CI. **Scope is deliberate**: structured config only, where a path is operative. Prose docs are NOT checked - 52 of 137 backticked file tokens in CLAUDE.md do not resolve as literal paths (bare basenames, documented-as-deleted files, `~/` paths, gitignored dirs), so a prose gate would sit at ~38% false positives and get muted. That is also why no repo-wide grep for the retired names exists: CHANGELOG/PROGRESS/ROADMAP/docs-plans legitimately contain them as history. Stdlib only. Tests at `tests/test_live_config.py` include regression cases replaying the real pre-fix CODEOWNERS and dependabot.yml. |
...
- **Custom voice schema** (v3.7.1+): `~/.creators-studio/config.json` `custom_voices.{role}` is the canonical location (v4.2.2+; auto-migrates from `~/.banana/config.json`). Role-keyed (narrator, character_a, etc.), supports three creation paths via `source_type` discriminator (designed | cloned | library), provider-pluggable for future second-provider support.
...
- **v4.0.0 backward-compat boundary** (**SUPERSEDED in v4.2.2** - see next bullet): originally documented that `~/.banana/` was intentionally NOT renamed to avoid forcing users to re-paste API keys. v4.2.2 introduced auto-migration that preserves user state, so the rename happens safely. **`@ycse/nanobanana-mcp` package name remains unchanged** - it's a third-party upstream dependency. **Product branding can be renamed freely; user state is renamed safely via auto-migration.**

- **v4.2.2 config dir auto-migration** (`~/.banana/` → `~/.creators-studio/`): the user state directory now matches the v4.0.0 plugin rebrand. Single source of truth: `scripts/paths.py::creators_studio_dir()`. On first call after upgrading, the helper does `shutil.copytree(old, new, symlinks=True)` and returns the new path. **Old directory is preserved (never deleted)** - copy, not move - so v4.2.1 plugins on the same machine still read it, and users have a fallback if anything goes wrong. Convenience accessors (`config_path()`, `costs_path()`, `presets_dir()`, etc.) wrap the migration trigger so call sites just import what they need. Tests at `tests/test_paths.py` cover all four state-graph corners (neither / old-only / new-only / both) plus symlink preservation and copy-failure fallback. Users can run `python3 skills/create-image/scripts/validate_setup.py --check-migration` to inspect their current state. After verifying the new path works, users can manually `rm -rf ~/.banana/` - the plugin never auto-deletes it.

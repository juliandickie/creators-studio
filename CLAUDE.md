...
| `skills/create-transcript/scripts/transcribe.py` | **v4.3.0** Scribe v2 speech-to-text CLI. Reuses the ElevenLabs key, `_http_post_multipart` (repeated `keyterms` fields), ffprobe/ffmpeg mono-16kHz prep (fail-loud on no-audio), one STT call, JSON cache, render dispatch, plus `rename`/`cost`/`status` subcommands and batch. Stdlib only. Pure helpers (auth/keyterms/probe/multipart) are unit-tested. |

...
| 22 | **Provider abstraction foundation (sub-project A)** | v4.2.0 | `ProviderBackend` ABC + canonical task schema (9 task types) + model registry at `scripts/registry/models.json` + two-stage routing (`scripts/routing.py`). Replicate backend refactored as the first concrete `ProviderBackend`. Config migration shim so old flat keys (`replicate_api_token`, etc.) keep working. Python floor lifted from 3.6+ to 3.12+ (PEP 604 unions, built-in generics, `dataclass(slots=True)`, `match`/`case`). Test suite introduced (stdlib `unittest`, 74 tests). Zero behavior change - adds the plumbing for marketplaces B/C/D to ship as single-file additions. Spec: `docs/superpowers/specs/2026-04-23-provider-abstraction-design.md`. |
| 23 | **`/create-transcript` - speech-to-text (ElevenLabs Scribe v2)** | v4.3.0 | Third top-level skill and the plugin's first ingest capability. Diarized markdown / SRT / VTT / YouTube chapters / plaintext, all pure-rendered from one cached API call (speaker renames and added formats never re-charge). Keyterm prompting (three-tier merge), batch mode, `rename` / `cost` / `status`. `scribe-v2` registered (`transcription` family, subscription / `rate: null` - no fabricated price). 34 new tests. Spec: `docs/superpowers/specs/2026-07-23-create-transcript-skill-design.md`. |
| 24 | **Keyterm sets for `/create-transcript`** | v4.4.0 | Named per-video keyterm vocabularies (`transcription.keyterm_sets`) activated with `--keyterm-set <name>`, so brand bias and its +20% surcharge apply only to the videos that need it. Ad-hoc `--keyterms` unions on top; `--keyterms-replace` = only the terms named this run. Unknown set fails loud; `status` lists sets. Backwards-compatible. 7 new tests (217 total). |
| 25 | **Descriptive transcript titles** | v4.5.0 | Transcripts lead with a meaningful title instead of the video filename. `retitle --json <cache> --title "..."` sets the markdown H1 and prefixes output filenames with `<title> - ` (video name kept at the end for search); `transcribe --title` does it in one pass. Re-renders from cache, no API call; `_title`/`_source_name` persisted so re-runs are idempotent. SKILL.md applies a title on every transcription. 5 new tests (222 total). |
...
| 10k | v4.1.0 - **ImageMagick awareness + Recraft Vectorize + exact-dim social enforcer**. Three upgrades: (1) `/create-image vectorize` via Recraft Vectorize ($0.01/call) closes the AI-logo-to-scalable-SVG gap; (2) `resize_for_platform()` inspects Gemini output dimensions, detects ratio drift, resizes+crops to exact platform specs (replaces silent "copy unchanged" fallback); (3) install-prompt UX across validate_setup / multiformat / SKILL.md §9.5 / setup.md / README Requirements - teaches users which tools unlock which features with explicit 3-option choices rather than silent degradation. | Medium | High | **✅ Shipped 2026-04-17** |
| 10l | v4.1.1 - **Platform count honesty + max-quality upload specs**. Narrowed `/create-image social` scope from 11 platforms (46 shallow placements) to 6 with deep coverage (38 placements across Instagram, Facebook, YouTube, LinkedIn, Twitter/X, TikTok). Every pixel spec upgraded to SOP max-quality (YouTube thumb 1280×720 → 3840×2160, 9× pixel count). Fixed x-header ratio bug (3:2 label → 3:1 true via 21:9 generation + crop). Retired Pinterest/Threads/Snapchat/Google Ads/Spotify keys (breaking). | Small | High | **✅ Shipped 2026-04-17** |
| 10m | v4.1.2 - **Full 16-platform coverage + plugin-wide text-by-default**. Restored v4.1.1-dropped platforms (Pinterest, Threads, Snapchat, Google Ads, Spotify) AND added Telegram, Signal, WhatsApp, ManyChat, BlueSky for marketer messaging/broadcast coverage. **87 image placements across 16 platforms** at SOP max-quality specs. Default `--mode` flipped from `image-only` to `complete` on `/create-image social`; `--mode image-only` now actually suppresses text in the prompt. Video-side data captured as reference in new `skills/create-video/references/social-platforms.md` (37 video placements × 14 platforms with duration ranges) - data-only in v4.1.2, wired to CLI in v4.2.0. | Medium | High | **✅ Shipped 2026-04-17** |
| 10n | v4.2.0 - **Provider abstraction foundation (sub-project A)**. Architectural prerequisite that supersedes the originally-planned `/create-video social` work for this version slot. Ships `ProviderBackend` ABC + canonical task schema + model registry at `scripts/registry/models.json` + two-stage routing. Replicate refactored to implement the new interface. Python floor lifted 3.6→3.12. Zero behavior change; makes sub-projects B/C/D ship as single-file additions. Spec: `docs/superpowers/specs/2026-04-23-provider-abstraction-design.md`. Plan: `docs/superpowers/plans/2026-04-23-provider-abstraction-subproject-a.md`. | Large | Very High | **✅ Shipped 2026-04-23** |
| 10n-B | v4.2.1 - **Vertex retirement + Lyria 3 upgrade (sub-project B)**. Delete `_vertex_backend.py` (958 lines). Route VEO 3.1 (Lite/Fast/Standard) through `scripts/backends/_replicate.py`. Migrate `audio_pipeline.py` Lyria code from inline Vertex URL construction to `ReplicateBackend`. Upgrade Lyria 2 → Lyria 3 as within-Lyria default; keep Lyria 2 for `negative_prompt` workflows; register Lyria 3 Pro for full-song generation. Register ElevenLabs Music with `(direct)` sentinel slug. Three new pricing modes in `cost_tracker.py`. Kling v3 + v3 Omni pricing corrected (v4.2.0 figures were wrong). `--confirm-upgrade` hard gate prevents silent 2x cost surprises on Lyria 3 Pro auto-routing. Multi-model principle codified. 137 tests passing (was 74). Zero user-visible behavior change except Lyria default upgrade. | Large | High | **✅ Shipped 2026-04-24** |
...
| 10n-motion | v4.3.x+ - **Register Kling 3.0 motion-control**. `kwaivgi/kling-v3-motion-control` - transfers motion from a reference video onto a character from a reference image. \$0.07/s (std) or \$0.12/s (pro). Different canonical task shape than existing image-to-video; needs canonical-schema extension. Out of scope for B; queued as a future candidate. | Small | Low | Queued |
| 10n-B2 | **v4.2.2 - Config directory rename `~/.banana/` → `~/.creators-studio/`**. ✅ **SHIPPED 2026-04-27.** Centralized in `scripts/paths.py` (single source of truth). 19 Python scripts refactored to use `creators_studio_dir()` accessor. Migration is COPY (not move) - old directory preserved indefinitely so older plugin versions on the same machine continue to work AND users have manual recovery fallback. 17 unit tests cover all 4 state-graph corners (none / old-only / new-only / both) plus symlink preservation and copy-failure fallback. New `validate_setup.py --check-migration` subcommand for users to inspect state. CLAUDE.md backward-compat boundary note rewritten. | Small | Medium | **DONE** |
| 10n-C | v4.3.0 - **Kie.ai backend (sub-project C)**. New `scripts/backends/_kie.py` implementing `ProviderBackend`. Unlocks Suno music (Kie-exclusive - Replicate lacks Suno). Users with existing Kie.ai billing can use `--provider kie` for Kling, VEO, Sora 2, etc. without creating a Replicate account. Adds Kie pricing mode (credit-based) to `cost_tracker.py`. | Medium | High | Planned |
| 10n-D | v4.4.0+ - **Hugging Face Inference Providers backend (sub-project D)**. One `scripts/backends/_hf.py` file unlocks ~17 underlying inference providers (Fal, Together, Replicate via HF, Groq, Fireworks, etc.) for `text-to-image` and `text-to-video` tasks. Gated on confirming HF supports `image-to-video` specifically (the plugin relies heavily on `start_image`). Not blocking. | Medium | Medium | Future |
| 10o | v4.2.x / v4.3.x - **`/create-video social` command**. Wires the v4.1.2 video spec catalogue to a CLI. Will generate platform-native videos via Kling v3 Std (≤15s default), Kling shot-list pipeline (>15s), or VEO 3.1 backup. New `skills/create-video/scripts/social.py` with PLATFORMS_VIDEO dict importing from the reference doc. Duration-aware model routing. Was originally the v4.2.0 target but bumped to make room for the provider-abstraction foundation, which is a prerequisite for multi-provider social pipelines. | Large | High | Planned |
| 10p | v4.3.0+ - **`/social-pack` unified command**. Combines image + video generation for a platform in a single call. User picks a platform (e.g. `instagram`), gets feed post + story + reel + reel video + profile all produced in one orchestrated batch. Coordinates `/create-image social` and `/create-video social` under a common CLI. | Large | Medium | Future |

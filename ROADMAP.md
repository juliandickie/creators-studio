...
| 10n-B | v4.2.1 — **Vertex retirement via Replicate routing (sub-project B)**. Route VEO 3.1 (all tiers: Lite/Fast/Standard) + Lyria 3 through `scripts/backends/_replicate.py` using `google/veo-3.1-*` and `google/lyria-3` slugs. Delete `_vertex_backend.py`. Remove `vertex_*` setup flow (config migration shim continues to read legacy keys). Separate plan applies the v4.2.0 spec. | Small | Medium | **Next** |

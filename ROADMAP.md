...
| 10n-B2 | v4.2.2 — **Config directory rename `~/.banana/` → `~/.creators-studio/`**. Cosmetic tech debt from the v4.0.0 rebrand era — `~/.banana/` was deliberately preserved in v4.0.0 to avoid doubling breaking-change surface during the rename. Now that the plugin has been `creators-studio` for several releases, finish the job. Plan: centralize `CONFIG_DIR`/`CONFIG_PATH` constants into one shared `scripts/config.py` module, update every caller to import from it, add auto-migration logic (if `~/.banana/` exists and `~/.creators-studio/` doesn't → move contents; if both exist → prefer new, leave old as `.banana.bak` for safety, log clearly). Update CLAUDE.md's v4.0.0 config-boundary note from "intentionally NOT renamed" to "migrated in v4.2.2." Small, focused release; dedicated test matrix for the migration logic. | Small | Medium | Queued |

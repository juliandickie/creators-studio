...
| 10a | v3.8.x — **Seedance 2.0 retest with Phase 2 subjects and avatars** — user-requested follow-up. Phase 1 rejected Seedance on safety filter; Phase 2 subjects (woman in home office, athlete, fashion model, robot mascot, etc.) may handle it differently. Estimated $2-3 for a focused retest. See `spikes/v3.8.0-provider-bakeoff/NEXT_SESSION.md` → "Deferred to v3.8.x" section for the retest plan. | Small | Medium | After v3.8.0 ships |
| 10b | v3.8.x — `_vertex_smoke_test.py` pre-flight check — validates all 5 Vertex API constraints discovered in spike 5 Phase 2 (preview vs GA IDs, Scene Ext v2 720p forced, 15 MB inline upload cap, duration {4,6,8}, aspect_ratio {16:9, 9:16}). Add to `skills/video/scripts/` next to `_vertex_backend.py`. Prevents users from hitting the same constraints we did. | Small | Medium | After v3.8.0 ships |
